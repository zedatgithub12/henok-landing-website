import Image from "next/image";
import React from "react";
interface IntegratedApps {
  id: string;
  name: string;
  logo: string;
}

const integrations: IntegratedApps[] = [
  {
    id: "slack",
    name: "Slack",
    logo: "/images/Slack.png",
  },
  {
    id: "twilio",
    name: "Twilio",
    logo: "/images/twillo.png",
  },
  {
    id: "discord",
    name: "Discord",
    logo: "/images/Discord.png",
  },
  {
    id: "drive",
    name: "Drive",
    logo: "/images/Drive.png",
  },
];

const Integrations = () => {
  return (
    <div className="min-h-screen w-full bg-black text-center mt-40  relative">
      <div className="max-w-6xl w-full mx-auto bg-black px-12 py-10 flex items-center justify-between absolute -top-20 left-16 rounded-4xl">
        <div className="text-left">
          <h1 className="text-2xl md:text-3xl font-semibold text-white leading-tight">
            Integrations
          </h1>
          <p className="text-gray-50 text-[16px] leading-relaxed max-w-xl text-left text-xs mt-2 font-light">
            Connect Auxi to your enterprise stack in minutes, allowing your team
            to instantly experience a 10x productivity improvement without
            compromising on security or ease of use.
          </p>
        </div>

        <div className="flex items-center justify-center gap-6">
          {integrations.map((app) => (
            <div
              key={app.id}
              className="flex flex-col items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity"
            >
              <div
                className={`w-16 h-16 rounded-lg flex items-center justify-center shadow-lg`}
              >
                <Image
                  src={app.logo || "/placeholder.svg"}
                  alt={`${app.name} logo`}
                  width={200}
                  height={200}
                  className="w-12 h-12 rounded"
                />
              </div>
              <span className="text-white text-sm font-medium">{app.name}</span>
            </div>
          ))}
        </div>
      </div>

      <div className="max-w-6xl w-full mx-auto  px-10 pb-16">
        <div
          className="py-4"
          style={{
            background:
              "radial-gradient(circle,  #333 0%, #000 80%, #000 100%)",
          }}
        >
          <Image
            src="/images/integration-pattern.png"
            alt="pattern"
            width={5000}
            height={5000}
            className="max-w-3xl w-full h-96 object-cover object-center mt-6 "
          />
        </div>
        <Image
          src="/images/chat-dashboard.png"
          alt="Dashboard"
          width={1000}
          height={1000}
          className="w-full h-full object-contain -mt-46"
        />
      </div>
    </div>
  );
};

export default Integrations;
